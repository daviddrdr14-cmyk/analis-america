...
# MAPA SIMPLE - SIN CARACTERES RAROS
MAPA = {
    "america": "America",
    "chivas": "Guadalajara",
    "boca": "Boca Juniors",
...
def get_stats_universal(equipo_id):
    for params in [
        {"team": equipo_id, "last": 5},
        {"team": equipo_id, "last": 5, "season": 2024},
        {"team": equipo_id, "last": 5, "season": 2023},
    ]:
        try:
            r = requests.get("https://v3.football.api-sports.io/fixtures", headers=HEADERS, params=params, timeout=15)
...
def analizar(texto):
    ...
    try:
        ...
        over15 = 85 if prom1 + prom2 > 2.5 else 70
        over25 = 68 if prom1 + prom2 > 3.0 else 50
